 ...
 *
 ---------------------------------------------------------------------------- */
    ...
    //#region department manager (select,insert,update,delete)
    

    deleteDepartment(o_params){
        try {
            var _id = o_params._id;
            var deffered = this.q.defer();
            var that = this;

            var query = this.Department.remove({_id:_id});
            query.exec(function(err,departemnts){
                if (err){
    ...
    updateDepartments(o_params){
        ...
        try {
            ...
            var query = this.Department.find({_id:_id}).update(set);
    ...
    insertDepartment(o_params){
        ...
        try {
            ...
            var deffered = this.q.defer();
            var that = this;
            var query = this.Department.create(data,function (error, department) {
                    if (error) {
    ...
    getDepartmentsFromDB(o_params){
        ...
        try {
            this.setDb().then(function(response){
                ...
                var query = that.Department.find({})
                .select({
                    "_id":1
                    ,"name":1
                    ,"displayName":1
                ...
                query.exec(function(err,departments){
                    if (err){
                        ...
                        deffered.resolve({status:true,data:departments,msg:" [x] A felhasználók lekérdezése sikeresen megtörtént " });
                    }
                });
            },function(error){
    ...
    getDepartmentFromDB(o_params){
                
        var departmentParams = Object.assign({ 
            token:""
            ,email:""
            ,name:""
            ,_id:""
        ...
        try {
            this.setDb().then(function(response){
                ...
                jwt.verify(departmentParams.token, config.secretKey, function(err, decoded) {
                    if (err){
                        deffered.resolve({status:false,data:{},msg:" [x] Hiba felhasználó adatainak lekérése során " + err});
                    }else{
                        that.Department.find({},function(err,departments){
                            console.log(departments);
                        });

                        departmentId = ((departmentParams._id!="")? departmentParams._id:decoded.id);
                        var query =  Department.findOne({ _id:departmentId });
                            ...
                            if (err){
                                ...
                                if (!department){
                                    ...
                                    deffered.resolve({status:true,data:department,msg:"Sikeres"});
                                }
                            }
                        });
                    }
    ...
    getView(o_params){
        ...
        try {
            ...
            if (params.action != 'insert') {
                this.getDepartmentFromDB(params).then(function(response){
                    //a departmentHiddenData kell a query-hez
                    if (response.status) {
                        departmentData =  {
